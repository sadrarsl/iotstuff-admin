...
// import Search from "src/pages-util/discountCategory/components/Search";
...
import {
  T_fetched_discount,
  T_discount_search,
} from "../../pages-util/discount/_core/_type";
import {
  delete_discount,
  fetch_all_discount,
  post_discount,
  update_discount,
} from "../../pages-util/discount/_core/_crud";
import {
  filled_data_validation,
  is_fetched_data_valid,
  is_filled_data_valid,
  is_filled_data_valid_edit,
} from "../../pages-util/discount/_core/_validator";
import {
  columns,
  options_core,
} from "../../pages-util/discount/_core/_config";
...
import Search from "../../pages-util/discount/components/Search";
import Table from "../../modules/table/Table";
import Modal from "../../modules/modal/Modal";
import Fields from "../../pages-util/discount/components/Fields";
...
export default (): ReactNode => {
  ...
  const [data, setData] = useState<T_fetched_discount[]>([]);
  // Declared To Hold Table's Searched Data
  const [searchData, setSearchData] = useState<T_discount_search>({});
  ...
  const fetch_data = (searchParams?: T_discount_search) => {
    setIsLoading(true);
    if (isRead) {
      fetch_all_discount(TOKEN, {
        ...searchParams,
  ...
  */
  ...
  const handleSearchOption = useDebouncedCallback(
    (name: "discountCategoryId", value: any) => {
      options_core[name].fetcher(
        TOKEN,
        searchOptions,
        setSearchOptions,
  ...
  const searchs = {
    discountCategoryId: (value: any) =>
      handleSearchOption("discountCategoryId", value),
  };

  // Table Search's Fields Changes Handler
  const changes = {
    discountCategoryId: (value: any) =>
      handleSearchData("discountCategoryId", value),
      ...
      handler: (props) => {
        ...
        if (isEdit) {
          if (is_filled_data_valid_edit(values)) {
            ...
            update_discount(TOKEN, id, values)
              .then(() => {
            ...
            <Table
              ...
              tableProps={{
                pagination: {
                  ...
                  locale: { discounts_per_page: instanceName },
                  discountRender: (
                    page: any,
                    type: any,
                    originalElement: any
