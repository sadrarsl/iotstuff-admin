...
// import Search from "src/pages-util/itemExtra/components/Search";
...
import {
  T_fetched_itemExtra,
  T_itemExtra_search,
} from "../../pages-util/itemExtra/_core/_type";
import {
  delete_itemExtra,
  fetch_all_itemExtra,
  post_itemExtra,
  update_itemExtra,
} from "../../pages-util/itemExtra/_core/_crud";
import {
  filled_data_validation,
  is_fetched_data_valid,
  is_filled_data_valid,
  is_filled_data_valid_edit,
} from "../../pages-util/itemExtra/_core/_validator";
import { columns, options_core } from "../../pages-util/itemExtra/_core/_config";
...
import Search from "../../pages-util/itemExtra/components/Search";
import Table from "../../modules/table/Table";
import Modal from "../../modules/modal/Modal";
import Fields from "../../pages-util/itemExtra/components/Fields";
...
export default (): ReactNode => {
  ...
  const [data, setData] = useState<T_fetched_itemExtra[]>([]);
  // Declared To Hold Table's Searched Data
  const [searchData, setSearchData] = useState<T_itemExtra_search>({});
  ...
  const instanceName = "آیتم های اضافی";
  const modelName = "itemExtra";
  ...
  const fetch_data = (searchParams?: T_itemExtra_search) => {
    setIsLoading(true);
    if (isRead) {
      fetch_all_itemExtra(TOKEN, {
        ...searchParams,
  ...
  */
  ...
  const handleSearchOption = useDebouncedCallback(
    (name: "itemExtraCategoryId", value: any) => {
      options_core[name].fetcher(
        TOKEN,
        searchOptions,
        setSearchOptions,
  ...
  const searchs = {
    itemExtraCategoryId: (value: any) => handleSearchOption("itemExtraCategoryId", value),
  };

  // Table Search's Fields Changes Handler
  const changes = {
    itemExtraCategoryId: (value: any) => handleSearchData("itemExtraCategoryId", value),
    id: (value: any) => handleSearchData("id", value),
    quantity: (value: any) => handleSearchData("quantity", value),
    price: (value: any) => handleSearchData("price", value),
    endedPrice: (value: any) => handleSearchData("endedPrice", value),
    extraPackagingCost: (value: any) =>
      handleSearchData("extraPackagingCost", value),
    name: (value: any) => handleSearchData("name", value),
  };
      ...
      handler: (props) => {
        ...
        if (isEdit) {
          if (is_filled_data_valid_edit(values)) {
            ...
            update_itemExtra(TOKEN, id, values)
              .then(() => {
            ...
            <Table
              ...
              tableProps={{
                pagination: {
                  ...
                  locale: { itemExtras_per_page: instanceName },
                  itemExtraRender: (page: any, type: any, originalElement: any) => {
                    switch (type) {
                      case "prev":
