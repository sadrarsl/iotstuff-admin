...
// import Search from "src/pages-util/itemCategoryCategory/components/Search";
import { LeftOutlined, PlusOutlined, RightOutlined } from "@ant-design/icons";

import { FormikValues } from "formik";
import {
  T_fetched_itemCategory,
  T_itemCategory_search,
} from "../../pages-util/itemCategory/_core/_type";
import {
  delete_itemCategory,
  fetch_all_itemCategory,
  post_itemCategory,
  update_itemCategory,
} from "../../pages-util/itemCategory/_core/_crud";
import {
  filled_data_validation,
  is_fetched_data_valid,
  is_filled_data_valid,
  is_filled_data_valid_edit,
} from "../../pages-util/itemCategory/_core/_validator";
import {
  columns,
  options_core,
} from "../../pages-util/itemCategory/_core/_config";
...
import Search from "../../pages-util/itemCategory/components/Search";
import Table from "../../modules/table/Table";
import Modal from "../../modules/modal/Modal";
import Fields from "../../pages-util/itemCategory/components/Fields";
...
export default (): ReactNode => {
  ...
  const [data, setData] = useState<T_fetched_itemCategory[]>([]);
  // Declared To Hold Table's Searched Data
  const [searchData, setSearchData] = useState<T_itemCategory_search>({});
  ...
  const instanceName = "دسته بندی آیتم";
  ...
  const fetch_data = (searchParams?: T_itemCategory_search) => {
    setIsLoading(true);
    if (isRead) {
      fetch_all_itemCategory(TOKEN, {
        ...searchParams,
  ...
  */
  const handleSearchData = (name: string, value: any) => {
    if (name === "itemCategoryCategoryId") {
  ...
  const handleSearchOption = useDebouncedCallback(
    (name: "itemCategoryCategoryId", value: any) => {
      options_core[name].fetcher(
        TOKEN,
        searchOptions,
        setSearchOptions,
  ...
  const searchs = {
    itemCategoryCategoryId: (value: any) =>
      handleSearchOption("itemCategoryCategoryId", value),
  };

  // Table Search's Fields Changes Handler
  const changes = {
    itemCategoryCategoryId: (value: any) =>
      handleSearchData("itemCategoryCategoryId", value),
      ...
      handler: (props) => {
        ...
        if (isEdit) {
          if (is_filled_data_valid_edit(values)) {
            ...
            update_itemCategory(TOKEN, id, values)
              .then(() => {
          ...
          if (is_filled_data_valid(values)) {
            post_itemCategory(TOKEN, values)
              .then(() => {
            ...
            <Table
              ...
              tableProps={{
                pagination: {
                  ...
                  locale: { itemCategorys_per_page: instanceName },
                  itemCategoryRender: (
                    page: any,
                    type: any,
                    originalElement: any
