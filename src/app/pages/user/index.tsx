...
// import Search from "src/pages-util/user/components/Search";
...
import {
  T_fetched_user,
  T_user_search,
} from "../../pages-util/user/_core/_type";
import {
  delete_user,
  fetch_all_user,
  post_user,
  update_user,
} from "../../pages-util/user/_core/_crud";
import {
  filled_data_validation,
  is_fetched_data_valid,
  is_filled_data_valid,
  is_filled_data_valid_edit,
} from "../../pages-util/user/_core/_validator";
import { columns, options_core } from "../../pages-util/user/_core/_config";
...
import Search from "../../pages-util/user/components/Search";
import Table from "../../modules/table/Table";
import Modal from "../../modules/modal/Modal";
import Fields from "../../pages-util/user/components/Fields";
...
export default (): ReactNode => {
  ...
  const [data, setData] = useState<T_fetched_user[]>([]);
  // Declared To Hold Table's Searched Data
  const [searchData, setSearchData] = useState<T_user_search>({});
  ...
  const [isPSWChange, setIsPSWChange] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    setIsPSWChange(auth.searchPermission(`changepsw_${modelName}`));
  ...
  const fetch_data = (searchParams?: T_user_search) => {
    setIsLoading(true);
    if (isRead) {
      fetch_all_user(TOKEN, {
        ...searchParams,
  ...
  */
  ...
  const searchs = {
    // userCategoryId: (value: any) => handleSearchOption("userCategoryId", value),
  };

  // Table Search's Fields Changes Handler
  const changes = {
    // userCategoryId: (value: any) => handleSearchData("userCategoryId", value),
    id: (value: any) => handleSearchData("id", value),
    email: (value: any) => handleSearchData("email", value),
    phoneNumber: (value: any) => handleSearchData("phoneNumber", value),
  };
            ...
            <Table
              ...
              tableProps={{
                pagination: {
                  ...
                  locale: { users_per_page: instanceName },
                  userRender: (page: any, type: any, originalElement: any) => {
                    switch (type) {
                      case "prev":
