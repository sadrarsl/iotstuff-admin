...
export default ({
      ...
      if (initState[name]) {
        if (typeof initState[name] === "object") {
          initState[name].forEach(async (element: any) => {
            ...
            options_core[name].fetcher(options, setOptions, {}, true);
          });
        } else {
          await options_core[name].fetcher(
            options,
        ...
        options_core[name].fetcher(options, setOptions, {}, false);
      }
    });
  };
  ...
  const exclusiveChange:Record<any,any> = {
    user_id: (value: any) => {
      options_core["user_id"].setter(options, setOptions, value);
    },
  };
  ...
  return (
    <FormGenerator
      ...
      fields={fields({ options, optionsSetter: setOptions, exclusiveChange })}
      initState={initState}
